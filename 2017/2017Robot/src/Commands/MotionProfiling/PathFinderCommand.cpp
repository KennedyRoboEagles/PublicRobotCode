...
PathFinderCommand::PathFinderCommand(double distanceX, double distanceY, double endHeading) {
	...
	updateNotifier = new Notifier(PathFinderCommand::callUpdate, this);
...
void PathFinderCommand::callUpdate(void* cmd) {
	printf("Updating\n");
	((PathFinderCommand*)cmd)->update();
}

// Called just before this Command rguns the first time
void PathFinderCommand::Initialize() {
	...
	this->maxVelocity = prefs->GetDouble("maxVelocity", DEFAULT_MAX_ALLOWED_VELOCITY);
	this->maxAcceleration = prefs->GetDouble("maxAcceleration", DEFAULT_MAX_ALLOWED_ACCELERATION);
	this->maxJerk = prefs->GetDouble("maxJerk", DEFAULT_MAX_ALLOWED_JERK);
	...
	logger->Log("  Left: kp: %lf, ki: %lf, kd: %lf, kv: %lf, ka: %lf\n",
			leftConfig.kp, leftConfig.ki, leftConfig.kd, leftConfig.kv, leftConfig.ka);
	logger->Log("  Right: kp: %lf, ki: %lf, kd: %lf, kv: %lf, ka: %lf\n",
			rightConfig.kp, rightConfig.ki, rightConfig.kd, rightConfig.kv, rightConfig.ka);
	logger->Log("\n");
#endif
...
void PathFinderCommand::StartWorkerThread()
{
...
void PathFinderCommand::Execute() {}


void PathFinderCommand::WorkerThread(PathFinderCommand *thisCommand)
{
...
//			currentPhysicalPose.GetRightDistance());
	...
	double goalHeading = r2d(leftFollower.heading);
...
//	chassis->GetRightMaster()->Set((rightFollower.output) - turn);
	chassis->TankDrive(leftFollower.output + turn, rightFollower.output-turn);
...
bool PathFinderCommand::IsFinished() {
...
void PathFinderCommand::End() {
	this->cleanUp();
}

// Called when another command which requires one or more of the same
// subsystems is scheduled to run
void PathFinderCommand::Interrupted() {
	this->cleanUp();
}

void PathFinderCommand::StopWorkerThread()
{
#ifdef	DEBUG_WORKER_THREAD
...
void PathFinderCommand::cleanUp() {
	//updateNotifier->Stop();
	StopWorkerThread();
...
void PathFinderCommand::DumpCandidate(TrajectoryCandidate candidate)
{
	logger->Log(separator);
...
void PathFinderCommand::DumpSegments(const char *description, Segment* ideal, Segment* segmentLeft, Segment* segmentRight, int length)
{
	logger->Log(separator);
...
void PathFinderCommand::DumpWaypoints(Waypoint *points, int pointLength)
{
	logger->Log(separator);
...
void PathFinderCommand::DumpPose(Pose *pose)
{
	logger->Log("Pose:\n");
...
void PathFinderCommand::DumpDistanceFollower(const char *label, DistanceFollower *follower, Segment* segments,
...
void PathFinderCommand::WriteSegmentsToCsv(Segment* trajectory, int segmentCount)
{
