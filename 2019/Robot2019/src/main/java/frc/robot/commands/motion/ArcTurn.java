...
public class ArcTurn extends Command {
    private final Logger log = LoggerFactory.getLogger("ArcTurn"); 
    ...
    private final double kMaxVelocity = 100; // Inch/sec
    private final double kMaxAcceleration = 10*30; // Inch/sec^2
    private final double kMaxJerk = 10*30; // Inch/sec^3
    ...
    private final Notifier notifier_ = new Notifier(() -> {
        ...
        log.info("Signal" +  signal.getLeft() + "," +  signal.getRight());
        chassis_.setOpenLoop(signal);
    });

    public ArcTurn(double angleDegrees) {
        this(Arrays.asList(
            new Waypoint(0, 0, 0),
            new Waypoint(Pathfinder.d2r(angleDegrees) * Constants.kDriveWheelTrackWidthInches/2.0, 0, 0)
        ));
    }
    
    public ArcTurn(List<Waypoint> points) {
        requires(chassis_);
